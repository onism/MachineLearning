...
class IndRNNCell(nn.Module):
    ...
    def IndRNNCell(self, input, hidden, w_ih, w_hh, b_ih=None, nonlinearity='tanh'):
    ...
    def forward(self, input, hx):
        func = self.IndRNNCell
        return func(input, hx, self.weight_ih, self.weight_hh, self.bias_ih, self.nonlinearity)

class IndRNN(nn.Module):
    """docstring for IndRNN"""
    ...
    def forward(self, x, hx=None):
        hidden =[]
        if hx is None:
            hx = T.autograd.Variable(x.data.new(self.n_layer, x.size(1), self.hidden_size).zero_(), requires_grad=False).contiguous()
